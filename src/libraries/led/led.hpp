...
private:
    ...
    void animation(void)
    {
        ...
        {
            this->pixels.setPixelColor(led, this->pallet[current_value]);
        }
        this->pixels.show();
    }
...
public:
    LEDThread(int data_pin, AnimationSettings *animation_settings, bool *event) : Thread()
    {
        ...
        this->event = event;
        this->pixels = Adafruit_NeoPixel(this->animation_settings->pixel_number, data_pin, NEO_GRB + NEO_KHZ800);
        this->setpoint_counter = 0;
        this->setpoint = 20;
    }
    void init(void)
    {
